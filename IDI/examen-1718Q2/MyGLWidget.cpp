...
void MyGLWidget::paintGL () 
{
  ...
  //pat 1
  modelTransformModel (0, 2);
  // Pintem Model
  glDrawArrays(GL_TRIANGLES, 0, m.faces().size()*3);

  //pat 2
  modelTransformModel (M_PI/2, 4);
  // Pintem Model
  glDrawArrays(GL_TRIANGLES, 0, m.faces().size()*3);

  //pat 3
  modelTransformModel (2*M_PI/2, 6);
  // Pintem Model
  glDrawArrays(GL_TRIANGLES, 0, m.faces().size()*3);

  //pat 4
  modelTransformModel (3*M_PI/2, 8);
  // Pintem Model
  glDrawArrays(GL_TRIANGLES, 0, m.faces().size()*3);
...
void MyGLWidget::modelTransformModel (float angle, float escala)
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::rotate(TG, angle, glm::vec3(0,1,0));
  TG = glm::rotate(TG, float(M_PI/4), glm::vec3(0,1,0));
  TG = glm::translate(TG, glm::vec3(0,0,-(sqrt(pow(8,2)+pow(8,2)))));
...
void MyGLWidget::projectTransform ()
{
  ...
  Proj = glm::perspective(float(M_PI/3.0), 1.0f, radiEsc, 16.f*radiEsc);
...
void MyGLWidget::viewTransform ()
{
  ...
  View = glm::translate(glm::mat4(1.f), glm::vec3(0, 0, -2.5*radiEsc));
