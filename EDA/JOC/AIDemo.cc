  ...
  virtual void play()
  {
    ...
    if (is_day())
    {
      ...
      { // Iterate over all builders
        ...
        { // No food nearby
          if (random(0, 3) <= 1)
          { // Do this with 25% probability [try to build in random position]
            bool some_dir = false;
            for (uint i = 0; not some_dir and i < dirs.size(); ++i)
            {
              ...
              Pos np = p + d;
              if (pos_ok(np))
              {               // pos_ok checks np is in the board, but here something else would be needed to make
          ...
          { // Do this with 75% probability [move to money if possible, otherwise randomly]
            Dir random_dir = dirs[random(0, 3)];
            if (pos_ok(p + random_dir) and (cell(p + random_dir).id == -1 or
                                            citizen(cell(p + random_dir).id).type == Builder))
            { // Move there if no citizen or builder
              move(id, random_dir);
    ...
    else
    {
      ...
      for (int id : w)
      { // Iterate over all warriors
        if (random(0, 9) < 6)
        { // 60 % probability move randomly, otherwise do nothing
          Dir random_dir = dirs[random(0, 3)];
          Pos p = citizen(id).pos;
