...
struct PLAYER_NAME : public Player
{
  ...
  const int next_pos_bazooka(const Pos &p, const int &id)
  {
    ...
    while (it != it_w)
    {
      ...
      if (h_b < min_dist)
      {
        if (strongestWeapon(cell(*it).weapon, cell(p).weapon) == cell(*it).weapon)
        {
          civilian_path[id].second = *it;
          min_dist = h_b;
        }
    ...
    if (is_day())
    {
      ...
      for (int i : handymen)
      {
        ...
        if (!civilian_path[i].first.empty())
        {
          ...
          if (cell(next_pos).id != -1 and citizen(cell(next_pos).id).player != me() and
              pos_ok(cell(p + opposite_direction(pos2dir(p, next_pos)))) and
              cell(p + opposite_direction(pos2dir(p, next_pos))).type == Street)
          {
            move(i, opposite_direction(pos2dir(p, next_pos)));
          }
          else if (pos_ok(next_pos) and cell(next_pos).type == Street and cell(next_pos).id == -1)
          {
